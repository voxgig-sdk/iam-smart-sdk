...
boru-driven command-line client **and** interactive REPL for the IamSmart
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iam-smart-cli)
make build
...
./iam-smart-cli list mobile_registration_point
./iam-smart-cli list registration_service_counter

# 5. Override the API base URL for a single call
IAM_SMART_BASE=https://api.example.com ./iam-smart-cli list mobile_registration_point

# 6. No arguments -> interactive REPL
./iam-smart-cli
iam-smart> list mobile_registration_point
iam-smart> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iam-smart-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/iam-smart-cli list mobile_registration_point
   ```
...
Run with no arguments to open a REPL (prompt `iam-smart>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `mobile_registration_point`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/iam-smart-cli`. |
...
(`./iam-smart-cli <expr>`) and interactively (the REPL), and expressions compose the
