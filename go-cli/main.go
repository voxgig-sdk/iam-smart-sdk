// Command IamSmart-cli is a CLI + REPL for the IamSmart SDK,
...
const prompt = "iam-smart"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "mobile_registration_point registration_service_counter self_registration_kiosk"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IAM_SMART_APIKEY (api key), IAM_SMART_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IAM_SMART_APIKEY carries the API key and
	// IAM_SMART_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IAM_SMART_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IAM_SMART_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIamSmartSDK(opts)
