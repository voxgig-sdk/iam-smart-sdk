...
[MCP](https://modelcontextprotocol.io) server exposing the IamSmart SDK as
two agent tools — `iam-smart_list` and `iam-smart_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iam-smart-mcp)
make build

# 2. Provide credentials via the environment
export IAM_SMART_APIKEY=sk_live_xxx
...
```jsonc
// iam-smart_list: first page of records
{ "entity": "mobile_registration_point" }
{ "entity": "mobile_registration_point", "query": { } }

// iam-smart_load: one record by id
{ "entity": "mobile_registration_point", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iam-smart-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user iam-smart \
     -- "$PWD"/dist/*/iam-smart-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `iam-smart_list` and `iam-smart_load` tools now appear
   in new sessions. Ask the agent to *"list mobile_registration_point using iam-smart"*
   and it calls `iam-smart_list` with `{"entity":"mobile_registration_point"}`.
...
```sh
export IAM_SMART_APIKEY=sk_live_xxx            # API key
export IAM_SMART_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `iam-smart_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `iam-smart_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IAM_SMART_APIKEY` | API key sent with every request. |
| `IAM_SMART_BASE` | Optional override of the API base URL. |
