...
type Args struct {
	Entity string         `json:"entity" jsonschema:"mobile_registration_point | registration_service_counter | self_registration_kiosk"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.IamSmartSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "iam-smart_list",
		Description: "List records from IamSmart. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "iam-smart_load",
		Description: "Load a single record from IamSmart. " +
...
func runOp(client *sdk.IamSmartSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.IamSmartSDK, name string) (sdk.IamSmartEntity, error) {
	switch strings.ToLower(name) {
	case "mobile_registration_point":
		return client.MobileRegistrationPoint(nil), nil
	case "registration_service_counter":
		return client.RegistrationServiceCounter(nil), nil
	case "self_registration_kiosk":
		return client.SelfRegistrationKiosk(nil), nil
