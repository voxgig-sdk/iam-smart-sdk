...
describe('RegistrationServiceCounterDirect', async () => {
  ...
  test('direct-list-registration_service_counter', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-registration_service_counter', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IAM_SMART_TEST_REGISTRATION_SERVICE_COUNTER_ENTID']
