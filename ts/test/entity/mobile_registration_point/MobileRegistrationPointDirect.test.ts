...
describe('MobileRegistrationPointDirect', async () => {
  ...
  test('direct-list-mobile_registration_point', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-mobile_registration_point', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IAM_SMART_TEST_MOBILE_REGISTRATION_POINT_ENTID']
