...
describe('SelfRegistrationKioskDirect', async () => {
  ...
  test('direct-list-self_registration_kiosk', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-self_registration_kiosk', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IAM_SMART_TEST_SELF_REGISTRATION_KIOSK_ENTID']
