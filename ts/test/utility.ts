...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.IAMSMART_TEST_LIVE ||
    'TRUE' === process.env.IAMSMART_TEST_OVERRIDE
  ) {
  ...
  m.IAMSMART_TEST_EXPLAIN = process.env.IAMSMART_TEST_EXPLAIN || m.IAMSMART_TEST_EXPLAIN
