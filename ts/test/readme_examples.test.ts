...
//     (\`new IamSmartSDK(...)\` / \`IamSmartSDK.test(...)\` become
//     \`IamSmartSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"mobile_registration_point":{"test01":{"id":"test01"}},"registration_service_counter":{"test01":{"id":"test01"}},"self_registration_kiosk":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IamSmartSDK(...)\` and any
// \`IamSmartSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IamSmartSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IamSmartSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
