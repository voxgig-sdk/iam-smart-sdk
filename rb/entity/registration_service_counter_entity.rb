...
class RegistrationServiceCounterEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "registration_service_counter"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    RegistrationServiceCounterEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IamSmartHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RegistrationServiceCounter, Hash] the current RegistrationServiceCounter data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IamSmartHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RegistrationServiceCounter fields)
  def match_get
  ...
  #
  # @param reqmatch [RegistrationServiceCounterListMatch, Hash, nil] match filter (any subset of
  #   RegistrationServiceCounter fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<RegistrationServiceCounter>, Array] the matching RegistrationServiceCounter items; raises IamSmartError on failure
