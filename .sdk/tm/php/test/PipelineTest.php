...
{
    private static function utility(): IamSmartUtility
    {
        return new IamSmartUtility();
    }
    ...
    private static function ctx(array $over = []): IamSmartContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IamSmartContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IamSmartOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IamSmartError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IamSmartOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IamSmartMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IamSmartOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IamSmartMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IamSmartOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IamSmartMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IamSmartSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IamSmartMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IamSmartResponse([]),
            'result' => new IamSmartResult([]),
        ]);
        [, $err] = IamSmartMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new IamSmartSpec([]),
            'response' => null,
            'result' => new IamSmartResult([]),
        ]);
        [, $err] = IamSmartMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IamSmartSpec(['step' => 's']),
            'response' => new IamSmartResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IamSmartResult([]),
        ]);
        [, $err] = IamSmartMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IamSmartSpec(['step' => 's']),
            'response' => new IamSmartResponse(self::resp(200, ['v' => 1])),
            'result' => new IamSmartResult([]),
        ]);
        [, $err] = IamSmartMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IamSmartSpec(['step' => 's']),
            'response' => new IamSmartResponse(self::resp(200, ['v' => 2])),
            'result' => new IamSmartResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IamSmartMakeResponse::call($ctx);
    ...
    {
        $preset = new IamSmartResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IamSmartSpec([]),
            'response' => new IamSmartResponse([]),
            'result' => new IamSmartResult([]),
        ...
        [$got, $err] = IamSmartMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IamSmartResult([])]);
        [, $err] = IamSmartMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IamSmartSpec([]), 'result' => null]);
        [, $err] = IamSmartMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IamSmartSpec(['step' => 's']),
            'result' => new IamSmartResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IamSmartOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IamSmartMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IamSmartSpec(['step' => 's']),
            'result' => new IamSmartResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IamSmartOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IamSmartMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IamSmartResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IamSmartSpec([]),
            'result' => new IamSmartResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IamSmartMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IamSmartMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IamSmartError('boom', 'boom');
        $utility->fetcher = function (IamSmartContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IamSmartSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IamSmartMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IamSmartContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IamSmartMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IamSmartResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IamSmartContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IamSmartSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IamSmartMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IamSmartResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IamSmartSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IamSmartMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IamSmartMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IamSmartMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IamSmartResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IamSmartDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IamSmartResult(['ok' => false])]);
        $this->expectException(IamSmartError::class);
        IamSmartDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IamSmartResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IamSmartMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        IamSmartMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IamSmartBaseFeature();
        $b = new IamSmartBaseFeature();
        IamSmartFeatureAdd::call($ctx, $a);
        IamSmartFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IamSmartBaseFeature
    {
        $f = new IamSmartBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IamSmartFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IamSmartContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IamSmartSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IamSmartPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IamSmartResponse(['headers' => null]),
            'result' => new IamSmartResult([]),
        ]);
        IamSmartResultHeaders::call($ctx);
