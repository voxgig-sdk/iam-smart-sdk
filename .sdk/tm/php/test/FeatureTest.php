...
class FtCtrl extends IamSmartControl
{
...
class FtClient
{
    ...
    private IamSmartUtility $utility;

    public function __construct(IamSmartUtility $utility, array $options)
    {
    ...
    public function get_utility(): IamSmartUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IamSmartContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IamSmartContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IamSmartContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IamSmartConfig::make_feature($name);
    ...
    private static function buildUrl(IamSmartSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IamSmartError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IamSmartResponse($fetched);
        }
    ...
    private function fail(IamSmartContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IamSmartContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
