...
class IamSmartMakeError
{
    public static function call(?IamSmartContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IamSmartContext([], null);
        }
        $op = $ctx->op ?? new IamSmartOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IamSmartResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IamSmartError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IamSmartError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IamSmartError) {
