...
class IamSmartNetsimFeature extends IamSmartBaseFeature
{
    ...
    private function _simulate(IamSmartContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(IamSmartContext $ctx, array $applied): void
    {
        $client = $this->client;
