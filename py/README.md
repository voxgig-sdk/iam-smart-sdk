...
The Python SDK for the IamSmart API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.MobileRegistrationPoint(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.IamSmart(None).load(
    {"id": "test01"}, None
)
