...
class TestSelfRegistrationKioskEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.SelfRegistrationKiosk(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.SelfRegistrationKiosk(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IAM_SMART_TEST_SELF_REGISTRATION_KIOSK_ENTID JSON to run live")
        client = setup["client"]
...
def _self_registration_kiosk_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IAM_SMART_TEST_SELF_REGISTRATION_KIOSK_ENTID"))
