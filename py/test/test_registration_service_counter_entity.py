...
class TestRegistrationServiceCounterEntity:
    ...
    def test_should_stream(self):
        ...
        base = IamSmartSDK.test(seed, None)
        seen = list(base.RegistrationServiceCounter(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = IamSmartSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.RegistrationServiceCounter(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IAM_SMART_TEST_REGISTRATION_SERVICE_COUNTER_ENTID JSON to run live")
        client = setup["client"]
...
def _registration_service_counter_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IAM_SMART_TEST_REGISTRATION_SERVICE_COUNTER_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("IAM_SMART_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("IAM_SMART_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("IAM_SMART_TEST_EXPLAIN") == "TRUE",
        "live": _live,
