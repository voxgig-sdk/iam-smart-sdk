...
class TestMobileRegistrationPointEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.MobileRegistrationPoint(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.MobileRegistrationPoint(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IAM_SMART_TEST_MOBILE_REGISTRATION_POINT_ENTID JSON to run live")
        client = setup["client"]
...
def _mobile_registration_point_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IAM_SMART_TEST_MOBILE_REGISTRATION_POINT_ENTID"))
