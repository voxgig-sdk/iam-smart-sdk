...
func TestRegistrationServiceCounterEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.RegistrationServiceCounter(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.RegistrationServiceCounter(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		registrationServiceCounterRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.registration_service_counter")))
...
func registration_service_counterBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IAM_SMART_TEST_REGISTRATION_SERVICE_COUNTER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IAM_SMART_TEST_REGISTRATION_SERVICE_COUNTER_ENTID"])
