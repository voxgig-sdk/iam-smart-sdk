...
func TestSelfRegistrationKioskEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.SelfRegistrationKiosk(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.SelfRegistrationKiosk(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IAM_SMART_TEST_SELF_REGISTRATION_KIOSK_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		selfRegistrationKioskRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.self_registration_kiosk")))
...
func self_registration_kioskBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IAM_SMART_TEST_SELF_REGISTRATION_KIOSK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IAM_SMART_TEST_SELF_REGISTRATION_KIOSK_ENTID"])
