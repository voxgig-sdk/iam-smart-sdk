...
import (
	...
	omni "github.com/voxgig-sdk/iam-smart-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IamSmartError); is {
				out := map[string]any{
					"name":    "IamSmartError",
...
func omniCtx(arg any, client *sdk.IamSmartSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IamSmartSDK); is {
			client = live
