...
The Golang SDK for the IamSmart API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/iam-smart-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/iam-smart-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/iam-smart-sdk/go=../iam-smart-sdk/go
```
...
mobileregistrationpoint, err := client.MobileRegistrationPoint(nil).Load(
    map[string]any{"id": "test01"}, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(mobileregistrationpoint) // the loaded mock data
```
...
```go
func NewIamSmartSDK(options map[string]any) *IamSmartSDK
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *IamSmartSDK
```
...
| `MobileRegistrationPoint` | `(data map[string]any) IamSmartEntity` | Create a MobileRegistrationPoint entity instance. |
| `RegistrationServiceCounter` | `(data map[string]any) IamSmartEntity` | Create a RegistrationServiceCounter entity instance. |
| `SelfRegistrationKiosk` | `(data map[string]any) IamSmartEntity` | Create a SelfRegistrationKiosk entity instance. |

### Entity interface (IamSmartEntity)

All entities implement the `IamSmartEntity` interface.
...
slice):
    ...
    mobileregistrationpoint, err := client.MobileRegistrationPoint(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil { /* handle */ }
    // mobileregistrationpoint is the loaded record
...
```
github.com/voxgig-sdk/iam-smart-sdk/go/
├── iam-smart.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/iam-smart-sdk/go`) re-exports everything needed
...
mobileregistrationpoint.Load(map[string]any{"id": "example_id"}, nil)
