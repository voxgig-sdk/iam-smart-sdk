...
The Lua SDK for the IamSmart API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:MobileRegistrationPoint(nil):list(nil, nil)
...
local result, err = client:IamSmart(nil):load(
  { id = "test01" }, nil
)
